...
class _ReportsScreenState extends State<ReportsScreen> {
  ...
  Future<void> _deleteReport(BuildContext ctx, String docId) async {
    final confirm = await showDialog<bool>(
      ...
      builder: (_) => AlertDialog(
        ...
        title: const Text('حذف البلاغ', style: TextStyle(color: _C.textPrimary)),
  ...
  _StatusMeta _meta(String status) {
    switch (status) {
      case 'in_progress':
        return _StatusMeta(_C.blue, Icons.autorenew_rounded, 'جاري', 'In Progress');
      case 'done':
      case 'completed':
        return _StatusMeta(_C.green, Icons.check_circle_rounded, 'منجز', 'Completed');
      default:
        return _StatusMeta(_C.orange, Icons.hourglass_top_rounded, 'معلق', 'Pending');
    }
  }
          ...
          return GestureDetector(
            ...
            child: AnimatedContainer(
              ...
              decoration: BoxDecoration(
                color: active ? f.$3.withOpacity(0.2) : _C.card,
                borderRadius: BorderRadius.circular(30),
                border: Border.all(
                  color: active ? f.$3.withOpacity(0.6) : _C.divider,
                ),
              ),
              child: Text(
                f.$2,
  ...
  // ── Report Card ──────────────────────────────
  Widget _reportCard(int index, DocumentSnapshot doc,
      Map<String, dynamic> data, String status, BuildContext ctx) {
    ...
    return TweenAnimationBuilder<double>(
      ...
      child: GestureDetector(
        onTap: () => Navigator.push(
          ctx,
          MaterialPageRoute(
              builder: (_) => ReportDetailsScreen(docId: doc.id)),
        ),
        child: Container(
              ...
              Container(
                ...
                decoration: BoxDecoration(
                  gradient: LinearGradient(
                    colors: [meta.color, meta.color.withOpacity(0.2)],
                  ),
  ...
  Widget build(BuildContext context) {
    Stream<QuerySnapshot> stream() {
      ...
      final q = _filter == 'completed'
          ? col.where('status', whereIn: ['completed', 'done'])
          : col.where('status', isEqualTo: _filter);
      return q.snapshots();
    }
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        const Text('لا توجد بلاغات',
                            style:
                                TextStyle(color: _C.textSub, fontSize: 15)),
                      ],
                    ),
                  );
                }
